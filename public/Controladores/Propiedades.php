...
$app->POST('/propiedades', function ($request, $response, $args){
    $datos = $request->getParsedBody();//datos del postman los guarda en datos (vienen de la $request)
    ...
    $date = DateTime::createFromFormat('Y-m-d', $datos['fecha_inicio_disponibilidad']); //creo un objeto dataTime con el formato esperado
    if ($date === false || $date->format('Y-m-d') != $datos['fecha_inicio_disponibilidad']){
        $payload = json_encode(['error' => "El campo fecha no es correcto", 'code' => 400]);
    ...
    try{
        ...
        //Este código PHP prepara una consulta SQL utilizando parámetros con nombre para realizar una selección (SELECT) de todos los campos (*) de la tabla 'localidades' donde el ID (id) sea igual al valor proporcionado en el parámetro con nombre ':localidad_id'.
        $consulta->bindParam(':localidad_id',$datos['localidad_id']);
        $consulta->execute();//stmt es lo mmicmo que consulta?

        if ($consulta->rowCount() == 0) {
            $payload = json_encode(['error' => "El ID de localidad no existe", 'code' => 404]);
            $response->getBody()->write($payload);
             return $response;
        }
        ...
        $consulta->execute();
          ...
          if ($consulta->rowCount() == 0) {
            $payload = json_encode(['error' => "El ID de tipo propiedes no existe", 'code' => 404]);
            $response->getBody()->write($payload);
            return $response;
          }
